...
// =============================================================
// Manuelle Status-Update-Mail an den Kunden
// =============================================================
export async function sendStatusUpdateEmail({ to, name, orderNumber, customerCode, status, customerMessage, priceTotal, trackingUrl }) {
  if (!isConfigured() || !to) return { skipped: true }
  const priceStr = eur(priceTotal)
  const html = wrap('Update zu deinem Auftrag', `
    <p>Hallo ${esc(name)},</p>
    <p>hier ein aktuelles Update zu deinem Druckauftrag:</p>
    ${codeBox('Auftragsnummer', orderNumber)}
    ${codeBox('Aktueller Status', status)}
    ${priceStr ? codeBox('Preis', priceStr) : ''}
    ${customerMessage ? `<div style="margin:12px 0;padding:12px 14px;background:#f4f0ff;border-left:3px solid #8b5cf6;border-radius:6px;font-size:14px;"><b>Nachricht von Jannik:</b><br/>${esc(customerMessage)}</div>` : ''}
    ${codeBox('Dein Auftragscode', customerCode)}
    ${linkButton(trackingUrl, 'Auftragsstatus ansehen')}
  `)
  const text = `Hallo ${name},\n\nUpdate zu deinem Auftrag:\n\nAuftragsnummer: ${orderNumber}\nStatus: ${status}\n${priceStr ? `Preis: ${priceStr}\n` : ''}${customerMessage ? `\nNachricht von Jannik: ${customerMessage}\n` : ''}Dein Auftragscode: ${customerCode}\n${trackingUrl ? `\nStatus ansehen: ${trackingUrl}\n` : ''}\nJanniks 3D-Druck Service`

  return getTransporter().sendMail({
    from: fromHeader(),
    to,
    subject: `Update · ${orderNumber} · ${status}`,
    text,
    html,
  })
}
